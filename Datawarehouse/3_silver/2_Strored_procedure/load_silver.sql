...
select * into ##crm_cust_info from bronze.crm_cust_info
UPDATE ##crm_cust_info
SET 
    cst_firstname = COALESCE(cst_firstname, t.max_firstname),
...
WHERE ##crm_cust_info.cst_id = t.cst_id;
		...
		SELECT 
			...
			COALESCE(sls_quantity, 1) AS sls_quantity,
			CASE 
				WHEN sls_price IS NULL OR sls_price <= 0 
					THEN sls_sales / NULLIF(COALESCE(sls_quantity, 1), 0) 
